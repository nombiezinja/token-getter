...
import (
	...
	"github.com/nombiezinja/spotify-cli/sperror"
)
...
func BuildRequestObject(request Request) (*http.Request, error) {
	...
	if err != nil {
		err = sperror.New("EncodingError", "Error constructing http request", err)
		return req, err
	}
...
func BuildResponse(res *http.Response) (*Response, error) {
	...
	if err != nil {
		err := sperror.New("EncodingError", "Error reading the response body", err)
		return nil, err
	}

	if res.StatusCode < 200 || res.StatusCode > 299 {
		err = sperror.New("LoginradiusRespondedWithError", "Received error response from Loginradius", errors.New(string(body)))
	} else {
		response = Response{
...
func (c *Client) Send(request Request) (*Response, error) {
	...
	if err != nil {
		err := sperror.New("MakeRequestError", "Error making the request", err)
		return nil, err
	}
...
func EncodeBody(body interface{}) (*bytes.Buffer, error) {
	...
	if encodeErr != nil {
		err := sperror.New("EncodingError", "Error encoding the request body", encodeErr)
		return nil, err
	}
